<div class="">
    <div class="p-2 flex flex-col border rounded justify-between w-100">
        <a href="{{route('products.product', ['id' => $this->product->id, 'name' => Illuminate\Support\Str::slug($this->product->product_name), 'category' => Illuminate\Support\Str::slug($this->product->category->name), 'status' => 1 ])}}" class="space-y-3 self-center mb-3">
            <img alt="img" src="{{asset(Illuminate\Support\Str::after($this->product->getFirstMedia('main')->getUrl(), 'http://localhost/'))}}" class="object-cover bg-gray-50 h-44 md:h-60 w-full">
            <p class="font-bold tracking-wider text-center text-xs sm:text-base">{{Illuminate\Support\Str::limit($this->product->product_name,25)}}</p>
        </a>
        ...
        <div class="space-y-3 w-100 mt-3">
            <div class="flex justify-center items-center">
                ...
                <h5 class="font-bold tracking-wider text-sm sm:text-base text-center">{{number_format($this->product->productattribute->price)}}</h5>
            </div>
            ...
            @else
                <a wire:click="addToCart({{$this->product->id}})" class="cursor-pointer flex flex-1 py-1 text-xs sm:text-sm md:text-base w-100 tracking-wider font-bold bg-secondary rounded-md justify-center text-white cursor-pointer">
